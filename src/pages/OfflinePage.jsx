import { Link } from "react-router-dom";
import usePageTitle from "../hooks/usePageTitle";

const OfflinePage = () => {
  usePageTitle("Offline");

  const handleRetry = () => {
    window.location.reload();
  };

  return (
    <section className="py-10 lg:py-20 h-screen flex items-center bg-gray-50">
      <div className="md:container w-full mx-auto px-4">
        <div className="max-w-lg mx-auto text-center">
          {/* Offline Illustration */}
          <div className="mb-8">
            <svg className="w-32 h-32 mx-auto text-primary/30" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M18.364 5.636a9 9 0 010 12.728m0 0l-2.829-2.829m2.829 2.829L21 21M15.536 8.464a5 5 0 010 7.072m0 0l-2.829-2.829m-4.243 2.829a4.978 4.978 0 01-1.414-2.83m-1.414 5.658a9 9 0 01-2.167-9.238m7.824 2.167a1 1 0 111.414 1.414" />
            </svg>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">You're Offline</h2>
          <p className="text-gray-600 mb-8 max-w-md mx-auto">
            It looks like you've lost your internet connection. Please check your network and try again.
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <button onClick={handleRetry} className="btn-primary">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              Try Again
            </button>
            <Link to="/" className="btn-outline">
              Go to Homepage
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
};

export default OfflinePage;
